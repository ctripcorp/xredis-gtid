...
proc region_wait_for_sync {region_info {maxtries 50} {delay 100}} {
    set master_name [dict get $region_info master_name]
    foreach node {redis1 redis2 keeper} {
        if {$node ne $master_name} {
            set rc [dict get $region_info $node client]
            wait_for_condition $maxtries $delay {
                [status $rc master_link_status] eq "up"
            } else {
...
start_server {tags {"xsync"} overrides {gtid-enabled yes gtid-xsync-max-gap 100000}} {
    ...
    test "xsync chaos: passive dr" {
        ...
        for {set i 0} {$i < 10} {incr i} {
            ...
            region_wait_for_sync $A_info 300 100
            region_wait_for_sync $B_info 300 100
